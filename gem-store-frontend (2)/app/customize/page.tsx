...
export default function CustomizePage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // shipping_address JSON); a 401 means not logged in — either way,
      // surface the server's actual reason rather than a generic
      // "something went wrong." There's no client-side pre-check for
      // "am I logged in" anymore (an HttpOnly cookie can't be inspected
      // by JS) — this catch block is what handles that case now, via
      // the 401 the API returns.
